...
function historyTone(item, accounting) {
  ...
  // NFT — mavi ton
  const text = `${item.title} ${item.subtitle || ""} ${item.value || ""}`.toLowerCase();
  if (item.kind === "nft" || /nft|erc-?721|erc-?1155|collectible/.test(text)) return "nft";
  // Token transfer yönüne göre
  if (item.kind === "token") {
    if (item.direction === "incoming") return "incoming-token";
    if (item.direction === "outgoing") return "outgoing-token";
  }
  // Genel yön bazlı
  if (item.direction === "incoming") return "incoming";
  if (item.direction === "outgoing") return "outgoing";
  return "neutral";
}
...
async function printMonthlyReport() {
  try {
    ...
    const fmt = (n) => {
      ...
      const parts = Number(n).toFixed(4).split(".");
